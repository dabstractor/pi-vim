...
`syncBorderColorWithMode`: `false` (default) leaves Pi's thinking border untouched; `true` always recolors the border per mode; `"inherit"` recolors per mode only when Pi is showing its neutral resting ("thinking off") color, and otherwise defers to whatever the host is showing. With `"inherit"`, raising thinking to any level or letting another extension set a non-default border keeps that color instead of clobbering it, so you never lose the signal. Detection is an exact match against Pi's `thinkingOff` color, not a saturation guess, so it correctly leaves the gray `minimal` level alone too.

Under `"inherit"`, both the border and the mode label follow a token-based precedence: `borderMuted > thinking > accent` (or any other token). A mode set to `borderMuted` always paints muted, even while thinking is on; any other token defers to the active thinking color and falls back to its own color when thinking is off. The rule keys on the token, not the mode name, so whichever mode you assign `borderMuted` keeps the muted color while the other mode tracks thinking. That keeps the border changing between insert and normal even with thinking raised.
